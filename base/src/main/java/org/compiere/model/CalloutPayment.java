		...
		GridField mField, Object value, Object oldValue)
	{
		...
		if (C_Invoice_ID != 0)
		{
			...
			try
			{
				...
				if (rs.next ())
				{
					currencyFromId = rs.getInt (2);
					openAmt = rs.getBigDecimal (3); // Set Invoice Open
					// Amount
					if (openAmt == null)
						openAmt = Env.ZERO;
				}
			...
			currencyFromId = order.getC_Currency_ID();
			openAmt = order.getGrandTotal();
		}
		log.fine ("Open=" + openAmt + ", C_Invoice_ID=" + C_Invoice_ID
		...
		if ((C_Currency_ID > 0 && currencyFromId > 0)
			|| colName.equals ("C_Currency_ID")
			|| colName.equals ("C_ConversionType_ID"))
		{
			log.fine ("InvCurrency=" + currencyFromId + ", PayCurrency="
				+ C_Currency_ID + ", Date=" + ConvDate + ", Type="
				+ C_ConversionType_ID);
			CurrencyRate = MConversionRate.getRate (currencyFromId,
			...
			//
			openAmt = openAmt.multiply (CurrencyRate).setScale (
		...
		else if (C_Invoice_ID == 0)
		{
			...
			{
				OverUnderAmt = openAmt.subtract (PayAmt).subtract (
				...
				&& (!processed))
			{
				WriteOffAmt = openAmt.subtract (PayAmt).subtract (
			...
			// End By Goodwill
			{
				PayAmt = openAmt.subtract (DiscountAmt).subtract (
